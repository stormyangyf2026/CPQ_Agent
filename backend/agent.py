...
8. **报价单查询** — 使用 list_quotes 查询报价单列表（支持按单号和状态过滤）
9. **报价单生成** — 使用 create_quote 创建报价单（自动写入客户和BOM行项目）
...
### 流程二：报价单查询
1. 使用 list_quotes 查询报价单列表
2. 支持按报价单号（keyword）和状态（status）过滤
3. 返回结果包含 quoteId, quoteNo, accountName 等

### 流程三：报价单生成
1. 完成配置后，使用 search_customers 查找客户
2. 确认客户信息和报价行项
3. 使用 create_quote 创建报价单（一站式：头+行项目）
4. 返回报价单 ID 和编号（quoteNo）
